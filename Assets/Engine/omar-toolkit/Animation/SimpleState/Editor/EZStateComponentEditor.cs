...
namespace Oryx
{
    [CustomEditor(typeof(EZStateComponent))]
    public class EZStateComponentEditor : Editor
    {
        ...
        {
            EditorGUILayout.LabelField("Easy State Component - CLOSED BETA", EditorStyles.boldLabel);
            EZStateComponent fEzStateComponent = (EZStateComponent)target;
            if (fEzStateComponent.SimpleStateModels == null)
                fEzStateComponent.SimpleStateModels = new List<SimpleStateModel>();

            List<SimpleStateModel> simpleStateModels = fEzStateComponent.SimpleStateModels;
            ...
            if(_drawVersion1)
                */
            DrawVersioned(fEzStateComponent, simpleStateModels);
        }

        private void DrawVersioned(EZStateComponent fEzStateComponent, List<SimpleStateModel> simpleStateModels)
        {
            ...
            Animation component = fEzStateComponent.GetComponent<Animation>();
            ...
            for (var index = 0; index < simpleStateModels.Count; index++)
            {
                ...
                {
                    fEzStateComponent.PlayState(simpleStateModel.State, false, true);
                }
                ...
                if (_foldoutCache[simpleStateModel])
                {
                    ...
                    DrawEachElement(simpleStateModel, menu, fEzStateComponent);
            ...
            if (GUI.changed)
            {
                EditorUtility.SetDirty(fEzStateComponent);
            }
        }
        ...
        private void DrawEachElement(SimpleStateModel simpleStateModel, GenericMenu menu,
            EZStateComponent ezStateComponent)
        {
            EditorGUILayout.BeginHorizontal();
            ...
            for (var index = 0; index < simpleStateModel.BasicActionModels.Count; index++)
            {
                ...
                ActionType storedActionType = EZStateComponent.TypeToEnum[type];
                ActionType selectedActionType = (ActionType)EditorGUILayout.EnumPopup(storedActionType);
                Type selectedType = EZStateComponent.EnumToType[selectedActionType];
                ...
                EditorGUILayout.EndHorizontal();
                simpleStateActionModel = DrawEachStateAction(simpleStateActionModel, menu, ezStateComponent);
            }
            ...
            PlayExternalStateModel playExternalStateModel)
        {
            ...
            playExternalStateModel.Target = (EZStateComponent)EditorGUILayout.ObjectField("Target",
                playExternalStateModel.Target, typeof(EZStateComponent), true);
        ...
        private void DrawUpdateMultiCanvasGroupAction(UpdateMultiCanvasGroupActionModel updateMultiCanvasGroupActionModel)
        {
            ...
            {
                updateMultiCanvasGroupActionModel.CanvasGroups = ((EZStateComponent)target)
                    .GetComponentsInChildren<CanvasGroup>().ToList();
            }

            if (GUILayout.Button("F|W|Canvas Only"))
            {
                Canvas[] canvases = ((EZStateComponent)target).GetComponentsInChildren<Canvas>();
        ...
        private void DrawSetGraphicsRaycasters(SetGraphicsRaycasterActionModel setGraphicsRaycasterActionModel)
        {
            ...
            if (GUILayout.Button("F|All"))
            {
                setGraphicsRaycasterActionModel.Raycasters = ((EZStateComponent)target)
